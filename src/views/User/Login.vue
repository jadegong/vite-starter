  ...
  <div class="main login">
    <div class="login">
      <a-form :model="loginPageState.formParams" id="formLogin" class="user-layout-login" @finish="onFinish">
        ...
        <a-form-item name="password" :rules="[{ required: true, message: '请输入密码' }]" validateTrigger="blur">
          <a-input-password v-model:value="loginPageState.formParams.password" class="password" size="large" placeholder="请输入密码">
            <template #prefix>
              <lock-outlined />
            </template>
          </a-input-password>
        ...
        <a-form-item>
          <a-row :gutter="0">
            <a-col :span="16">
              <a-form-item name="captcha" :rules="[{ required: true, message: '请输入验证码' }]" validateTrigger="blur" no-style>
        ...
        <a-form-item style="margin-top: 24px">
          <a-button size="large" type="primary" html-type="submit" class="login-button" :loading="loginPageState.loginBtn" :disabled="loginPageState.loginBtn">登录</a-button>
        </a-form-item>
      </a-form>
    </div>
  </div>
